...
var addEventListener2 = (node, name, handler, options) => node.addEventListener(name, handler, options);
  ...
  #createAttribute(node, attribute3) {
    ...
    switch (kind) {
      ...
      case event_kind: {
        ...
        addEventListener2(node, name, handleEvent, { passive });
...
function scrollHeight(element4) {
  return element4.scrollHeight;
}
function setScrollTop(element4, value2) {
  element4.scrollTop = value2;
}
...
function utc_now() {
  ...
  return new Time(now$1, 0, new Some("Etc/UTC"), new Some(monotonic_now$1));
}
function compare4(a, b) {
  let wta;
  let mta;
  wta = a.wall_time;
  mta = a.monotonic_time;
  let wtb;
  let mtb;
  wtb = b.wall_time;
  mtb = b.monotonic_time;
  let _block;
...
function to_unix(value3) {
  ...
  return globalThis.Math.trunc(t / 1e6);
}
...
var map9 = map6;
function empty4(scaffold) {
  return element2(scaffold[0], scaffold[1], toList([]));
}
function text_content(scaffold, content) {
  return element2(scaffold[0], scaffold[1], toList([text2(content)]));
}
...
function add5(scaffold, attribute3) {
  return [scaffold[0], prepend(attribute3, scaffold[1])];
}
function class$2(scaffold, name2) {
  let _pipe = scaffold;
  return add5(_pipe, class$(name2));
}
function id2(scaffold, value3) {
  let _pipe = scaffold;
  return add5(_pipe, id(value3));
}
function disabled2(scaffold, is_disabled) {
  let _pipe = scaffold;
  return add5(_pipe, disabled(is_disabled));
}
function placeholder2(scaffold, text3) {
  let _pipe = scaffold;
  return add5(_pipe, placeholder(text3));
}
function type_2(scaffold, control_type) {
  let _pipe = scaffold;
  return add5(_pipe, type_(control_type));
}
function value3(scaffold, control_value) {
  let _pipe = scaffold;
  return add5(_pipe, value(control_value));
}
...
function on_input(message) {
  return on("input", subfield(toList(["target", "value"]), string2, (value4) => {
    return success(message(value4));
  }));
}
function formdata_decoder() {
  let string_value_decoder = field(0, string2, (key2) => {
    return field(1, one_of(map3(string2, (var0) => {
      return new Ok2(var0);
    }), toList([success(error_nil)])), (value4) => {
      let _pipe2 = value4;
      let _pipe$12 = map4(_pipe2, (_capture) => {
        return new$6(key2, _capture);
      });
...
function on_click2(scaffold, msg) {
  let _pipe = scaffold;
  return add5(_pipe, on_click(msg));
}
function on_input2(scaffold, msg) {
  let _pipe = scaffold;
  return add5(_pipe, on_input(msg));
}
function on_submit2(scaffold, msg) {
  let _pipe = scaffold;
  return add5(_pipe, on_submit(msg));
}
...
var empty5 = /* @__PURE__ */ new Uri(/* @__PURE__ */ new None, /* @__PURE__ */ new None, /* @__PURE__ */ new None, /* @__PURE__ */ new None, "", /* @__PURE__ */ new None, /* @__PURE__ */ new None);
...
function parse_host_outside_of_brackets_loop(loop$original, loop$uri_string, loop$pieces, loop$size) {
  while (true) {
    ...
    if (uri_string === "") {
      ...
      let host = string_codeunit_slice(original, 0, size4);
  ...
  while (true) {
    ...
    if (uri_string === "") {
      ...
      if (size4 === 0) {
        ...
        let rest = uri_string.slice(1);
        let host = string_codeunit_slice(original, 0, size4 + 1);
        let pieces$1 = new Uri(pieces.scheme, pieces.userinfo, new Some(host), pieces.port, pieces.path, pieces.query, pieces.fragment);
...
var ws_init = (url, on_open, on_text, on_close) => {
  if (typeof WebSocket === "function") {
    try {
      ...
      return Error2(new InvalidUrl(error.message));
    }
  } else {
    return Error2(new UnsupportedEnvironment("WebSocket global unavailable"));
  }
};
var ws_listen = (ws, on_open, on_text, on_close) => {
  ...
  ws.addEventListener("close", (event4) => on_close(event4.code, event4.reason ?? ""));
};
var ws_send = (ws, msg) => {
  ws.send(msg);
};
...
class ServiceRestart extends CustomType {
  constructor(code2, reason) {
    super();
    this.code = code2;
    this.reason = reason;
...
class TryAgainLater extends CustomType {
  constructor(code2, reason) {
    super();
    this.code = code2;
    this.reason = reason;
...
class BadGateway extends CustomType {
  constructor(code2, reason) {
    super();
    this.code = code2;
    this.reason = reason;
...
class UnsupportedEnvironment extends CustomType {
  constructor(message) {
    super();
    this.message = message;
  }
...
function parse_reason(code2, reason) {
  if (code2 === 1000) {
    return new Normal2(code2, reason);
  } else if (code2 === 1001) {
...
function do_get_websocket_path(path, page_uri) {
  ...
  _block = unwrap(_pipe, new Uri(new None, new None, new None, new None, path, new None, new None));
  let path_uri = _block;
  return try$(merge3(page_uri, path_uri), (merged) => {
...
function init(path) {
  ...
  if ($ instanceof Ok2) {
    ...
    return new Error2(new InvalidUrl("Invalid Url"));
  }
}
function listen(ws, on_open, on_text_message, on_close) {
  return ws_listen(ws, on_open, on_text_message, (code2, reason) => {
...
class TransportUp extends CustomType {
}
class TransportDown extends CustomType {
...
class TransportError extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class InitError extends CustomType {
  constructor(message) {
    super();
    this.message = message;
  }
...
class TransportHandlers extends CustomType {
...
function websocket(path) {
  ...
  if ($ instanceof Ok2) {
    let ws = $[0];
    return new Transport((handlers) => {
      return listen(ws, (_) => {
        return handlers.on_up();
      }, handlers.on_message, (reason) => {
...
class EncoderDecoder extends CustomType {
...
class CodedTransport extends CustomType {
  constructor(listen2, send3) {
    super();
    this.listen = listen2;
    this.send = send3;
...
function new_handlers(on_message, on_state, encoder_decoder) {
  return new TransportHandlers(() => {
    ...
    if ($ instanceof Ok2) {
      ...
      let decode_error2 = $[0];
      return on_state(new TransportError(new DecodeError2(decode_error2)));
    }
  }, (error) => {
...
function to_coded_transport(base, encoder_decoder) {
  return new CodedTransport((on_message, on_state) => {
    return base.listen(new_handlers(on_message, on_state, encoder_decoder));
  }, (msg, on_message, on_state) => {
    let $ = encoder_decoder.encode(msg);
    if ($ instanceof Ok2) {
      let msg$1 = $[0];
      return base.send(msg$1, new_handlers(on_message, on_state, encoder_decoder));
    } else {
      return;
    }
  });
}
function compose(init2, update2, transport, encoder_decoder, meta_wrapper) {
  let _block;
  let _pipe = transport;
  ...
  let omniinit = (flags) => {
    let _block$1;
    let _pipe$1 = (dispatch2) => {
      coded_transport.listen(dispatch2, (state) => {
        return dispatch2(meta_wrapper(state));
      });
      return;
    };
    _block$1 = from2(_pipe$1);
  ...
  let omniupdate = (model, msg) => {
    ...
    return [
      ...
      batch(toList([
        (() => {
          let _pipe$1 = (dispatch2) => {
            return coded_transport.send(msg, dispatch2, (state) => {
              return dispatch2(meta_wrapper(state));
            });
          };
          return from2(_pipe$1);
        })(),
        effect
      ]))
    ];
...
function component2(init2, update2, view, options, encoder_decoder, transport, transport_wrapper) {
  let $ = compose(init2, update2, transport, encoder_decoder, transport_wrapper);
  let omniinit;
  let omniupdate;
  omniinit = $[0];
  omniupdate = $[1];
  return component(omniinit, omniupdate, view, options);
}

// build/dev/javascript/gluid/gluid.mjs
function format_uuid(src2) {
  return slice(src2, 0, 8) + "-" + slice(src2, 8, 4) + "-" + slice(src2, 12, 4) + "-" + slice(src2, 16, 4) + "-" + slice(src2, 20, 12);
}
function guidv4() {
  let _block;
...
class AgentInfo extends CustomType {
  constructor(id3, name2, description) {
    super();
    this.id = id3;
    this.name = name2;
...
class ClientError extends CustomType {
}
class ServerError extends CustomType {
}
class Sent extends CustomType {
}
class Received extends CustomType {
}
class Sending extends CustomType {
}
class ChatMessage extends CustomType {
  constructor(id3, content2, status, sent_at, is_ai) {
    super();
    this.id = id3;
...
class SelectAgent extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UserSendChatMessage extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
class AgentSelected extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
class ServerUpsertChatMessages extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
class AgentList extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function chat_message_to_json(message) {
  return object2(toList([
    ["id", string3(message.id)],
    ["content", string3(message.content)],
    ["status", int3(encode_status(message.status))],
    ["sent_at", int3(to_unix(message.sent_at))],
    ["is_ai", bool2(message.is_ai)]
  ]));
}
function chat_message_decoder() {
  return field("id", string2, (id3) => {
    return field("content", string2, (content2) => {
      return field("status", status_decoder(), (status) => {
        return field("sent_at", int2, (sent_at_unix) => {
          return field("is_ai", bool, (is_ai) => {
            let sent_at = from_unix(sent_at_unix);
            return success(new ChatMessage(id3, content2, status, sent_at, is_ai));
          });
        });
      });
    });
...
function agent_info_decoder() {
  return field("id", string2, (id3) => {
    return field("name", string2, (name2) => {
      return field("description", string2, (description) => {
        return success(new AgentInfo(id3, name2, description));
      });
    });
  });
}
function encode_client_message(msg) {
  let _block;
  if (msg instanceof SelectAgent) {
    let agent_id = msg[0];
    _block = toList([int3(0), string3(agent_id)]);
  } else if (msg instanceof UserSendChatMessage) {
    let agent_id = msg[0];
    let chat_msg = msg[1];
    _block = toList([
      int3(1),
    ...
    let agent_id = msg[0];
    _block = toList([int3(2), string3(agent_id)]);
  }
  let _pipe = _block;
...
function server_message_decoder() {
  return field(0, int2, (id3) => {
    if (id3 === 0) {
      return field(1, string2, (agent_id) => {
        return field(2, list2(chat_message_decoder()), (messages) => {
          return success(new AgentSelected(agent_id, messages));
        });
      });
    } else if (id3 === 1) {
      return field(1, string2, (agent_id) => {
        return field(2, list2(chat_message_decoder()), (messages) => {
          return success(new ServerUpsertChatMessages(agent_id, messages));
        });
      });
    } else if (id3 === 2) {
      return field(1, list2(agent_info_decoder()), (agents) => {
        return success(new AgentList(agents));
      });
    } else {
      return failure(new AgentList(toList([])), "ServerMessage");
    }
  });
}
function decode_server_message(str_msg) {
  return parse(str_msg, server_message_decoder());
}
// build/dev/javascript/client/client/agents.mjs
class UserSelectAgent extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function agent_card(agent) {
  ...
  let emoji = _block;
  let _pipe = div();
  let _pipe$1 = class$2(_pipe, "border border-gray-200 rounded-xl p-6 flex flex-col items-center gap-4 hover:shadow-lg transition-shadow bg-white");
  return children(_pipe$1, toList([
    (() => {
      let _pipe$2 = div();
      let _pipe$3 = class$2(_pipe$2, "text-5xl");
      return text_content(_pipe$3, emoji);
    })(),
    (() => {
      let _pipe$2 = h2();
      let _pipe$3 = class$2(_pipe$2, "text-xl font-semibold text-gray-700");
      return text_content(_pipe$3, agent.name);
    })(),
    (() => {
      let _pipe$2 = p();
      let _pipe$3 = class$2(_pipe$2, "text-gray-500 text-center text-sm");
      return text_content(_pipe$3, agent.description);
    })(),
    (() => {
      let _pipe$2 = button();
      let _pipe$3 = class$2(_pipe$2, "mt-2 bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors");
      let _pipe$4 = on_click2(_pipe$3, new UserSelectAgent(agent.id));
      return text_content(_pipe$4, "Start Chat");
    })()
  ]));
}
function agent_selection_view(agents) {
  let _pipe = div();
  let _pipe$1 = class$2(_pipe, "h-full flex flex-col items-center justify-center gap-8 p-8");
  return children(_pipe$1, toList([
    (() => {
      let _pipe$2 = h1();
      let _pipe$3 = class$2(_pipe$2, "text-3xl font-bold text-gray-800");
      return text_content(_pipe$3, "Choose Your Agent");
    })(),
    (() => {
      let _pipe$2 = p();
      let _pipe$3 = class$2(_pipe$2, "text-gray-500");
      return text_content(_pipe$3, "Select an AI agent to start chatting");
    })(),
    (() => {
      let _pipe$2 = div();
      let _pipe$3 = class$2(_pipe$2, "grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl w-full");
      return children(_pipe$3, (() => {
        let _pipe$4 = agents;
        return map2(_pipe$4, agent_card);
      })());
    })()
  ]));
}
...
class Model extends CustomType {
  constructor(current_agent, messages, draft_content, agents, connected) {
    super();
...
class UserSendDraft extends CustomType {
}
class UserScrollToLatest extends CustomType {
}
class UserUpdateDraftContent extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UserSelectAgent2 extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class UserGoBack extends CustomType {
}
class ClientMessage extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class ServerMessage extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
}
class TransportState extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var hardcoded_agents = /* @__PURE__ */ toList([
  /* @__PURE__ */ new AgentInfo("elf", "Elara the Elf", "A wise high elf who speaks in archaic prose about magic and nature"),
  /* @__PURE__ */ new AgentInfo("marketer", "Marketing Maven", "A seasoned marketing expert who analyzes everything through a business lens"),
  /* @__PURE__ */ new AgentInfo("pm", "Project Maven", "An organized project manager who structures ideas into action plans")
]);
function chat_message_element(chat_msg) {
  ...
  let alignment = _block;
  let _block$1;
  let $1 = chat_msg.is_ai;
  if ($1) {
    _block$1 = "bg-gray-200 text-gray-800 rounded-2xl rounded-tl-sm";
  } else {
    _block$1 = "bg-blue-600 text-white rounded-2xl rounded-tr-sm";
  }
  let bubble_class = _block$1;
  let _block$2;
  let $2 = chat_msg.status;
  if ($2 instanceof ClientError) {
    _block$2 = " ❌";
  } else if ($2 instanceof ServerError) {
    _block$2 = " ⚠️";
  } else if ($2 instanceof Sending) {
    _block$2 = " \uD83D\uDD50";
  } else {
    _block$2 = "";
  }
  let status_indicator = _block$2;
  let _pipe = div();
  let _pipe$1 = class$2(_pipe, "flex w-full " + alignment);
  return children(_pipe$1, toList([
    (() => {
      let _pipe$2 = div();
      let _pipe$3 = class$2(_pipe$2, "max-w-[80%] p-3 " + bubble_class);
      return children(_pipe$3, toList([
        (() => {
          let _pipe$4 = p();
          let _pipe$5 = class$2(_pipe$4, "break-words");
          return text_content(_pipe$5, chat_msg.content + status_indicator);
        })()
      ]));
    })()
  ]));
...
function sort_chat_messages(chat_msgs) {
  return sort(chat_msgs, (msg_a, msg_b) => {
    return compare4(msg_a.sent_at, msg_b.sent_at);
  });
}
function view(model) {
  let $ = model.current_agent;
  if ($ instanceof Some) {
    let agent_id = $[0];
    let _block;
    let _pipe = model.agents;
    let _pipe$1 = find(_pipe, (a) => {
      return a.id === agent_id;
    });
    _block = unwrap(_pipe$1, new AgentInfo("", "Unknown", ""));
    let agent_info = _block;
    let _block$1;
    let _pipe$2 = model.messages;
    let _pipe$3 = values(_pipe$2);
    _block$1 = sort_chat_messages(_pipe$3);
    let sorted_chat_msgs = _block$1;
    let _pipe$4 = div();
    let _pipe$5 = class$2(_pipe$4, "h-full w-full flex flex-col bg-gray-50");
    return children(_pipe$5, toList([
      (() => {
        let _pipe$6 = div();
        let _pipe$7 = class$2(_pipe$6, "flex items-center gap-3 p-4 bg-white border-b border-gray-200 shadow-sm");
        return children(_pipe$7, toList([
          (() => {
            let _pipe$8 = button();
            let _pipe$9 = class$2(_pipe$8, "p-2 hover:bg-gray-100 rounded-lg transition-colors text-gray-600");
            let _pipe$10 = on_click2(_pipe$9, new UserGoBack);
            return children(_pipe$10, toList([
              (() => {
                let _pipe$11 = span();
                let _pipe$12 = class$2(_pipe$11, "text-xl");
                return text_content(_pipe$12, "←");
              })()
            ]));
          })(),
          (() => {
            let _pipe$8 = div();
            let _pipe$9 = class$2(_pipe$8, "flex items-center gap-2");
            return children(_pipe$9, toList([
              (() => {
                let _pipe$10 = div();
                let _pipe$11 = class$2(_pipe$10, "w-2 h-2 rounded-full");
                let _pipe$12 = class$2(_pipe$11, (() => {
                  let $1 = model.connected;
                  if ($1) {
                    return "bg-green-500";
                  } else {
                    return "bg-red-500";
                ...
                return empty4(_pipe$12);
              })(),
              (() => {
                let _pipe$10 = h1();
                let _pipe$11 = class$2(_pipe$10, "text-lg font-semibold text-gray-800");
                return text_content(_pipe$11, agent_info.name);
              })()
            ]));
          })()
        ]));
      ...
      (() => {
        let _pipe$6 = div();
        let _pipe$7 = id2(_pipe$6, msgs_container_id);
        let _pipe$8 = class$2(_pipe$7, "flex-1 overflow-y-auto p-4 space-y-3");
        return keyed(_pipe$8, map2(sorted_chat_msgs, (chat_msg) => {
          return [chat_msg.id, chat_message_element(chat_msg)];
        }));
      })(),
      (() => {
        let _pipe$6 = form2();
        let _pipe$7 = class$2(_pipe$6, "flex gap-3 p-4 bg-white border-t border-gray-200");
        let _pipe$8 = on_submit2(_pipe$7, (_) => {
          return new UserSendDraft;
        });
        return children(_pipe$8, toList([
          (() => {
            let _pipe$9 = input();
            let _pipe$10 = on_input2(_pipe$9, (var0) => {
              return new UserUpdateDraftContent(var0);
            });
            let _pipe$11 = type_2(_pipe$10, "text");
            let _pipe$12 = value3(_pipe$11, model.draft_content);
            let _pipe$13 = placeholder2(_pipe$12, "Type your message...");
            let _pipe$14 = class$2(_pipe$13, "flex-1 px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent");
            return empty4(_pipe$14);
          })(),
          (() => {
            let _pipe$9 = button();
            let _pipe$10 = type_2(_pipe$9, "submit");
            let _pipe$11 = disabled2(_pipe$10, trim(model.draft_content) === "" || model.connected === false);
            let _pipe$12 = class$2(_pipe$11, "px-6 py-3 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed");
            return text_content(_pipe$12, "Send");
          })()
        ]));
      })()
    ]));
    ...
    let _pipe = div();
    let _pipe$1 = class$2(_pipe, "h-full w-full bg-gray-50");
    return children(_pipe$1, toList([
      (() => {
        let _pipe$2 = agent_selection_view(model.agents);
        return map9(_pipe$2, (msg) => {
          let agent_id = msg[0];
          return new UserSelectAgent2(agent_id);
        });
      })()
    ]));
  }
...
function update2(model, msg) {
  if (msg instanceof UserSendDraft) {
    let $ = model.current_agent;
    if ($ instanceof Some) {
      let agent_id = $[0];
      let chat_msg = new_chat_msg(model.draft_content, new Sending);
      return [
        new Model(model.current_agent, insert(model.messages, chat_msg.id, chat_msg), "", model.agents, model.connected),
        from2((dispatch2) => {
          dispatch2(new ClientMessage(new UserSendChatMessage(agent_id, chat_msg)));
          return dispatch2(new UserScrollToLatest);
        })
      ];
    } else {
      return [model, none()];
  ...
  } else if (msg instanceof UserUpdateDraftContent) {
    let content2 = msg[0];
    return [
      new Model(model.current_agent, model.messages, content2, model.agents, model.connected),
      none()
    ];
  } else if (msg instanceof UserSelectAgent2) {
    let agent_id = msg[0];
    return [
      new Model(new Some(agent_id), model.messages, model.draft_content, model.agents, model.connected),
      from2((dispatch2) => {
        return dispatch2(new ClientMessage(new SelectAgent(agent_id)));
      })
    ];
  } else if (msg instanceof UserGoBack) {
    return [
      new Model(new None, model.messages, model.draft_content, model.agents, model.connected),
      none()
    ];
  } else if (msg instanceof ClientMessage) {
    let $ = msg[0];
    if ($ instanceof UserSendChatMessage) {
      let chat_msg = $[1];
      let messages = insert(model.messages, chat_msg.id, chat_msg);
      return [
        new Model(model.current_agent, messages, model.draft_content, model.agents, model.connected),
    ...
    let $ = msg[0];
    if ($ instanceof AgentSelected) {
      let agent_id = $[0];
      let messages = $[1];
      let _block;
      let _pipe = messages;
      let _pipe$1 = map2(_pipe, (m) => {
        return [m.id, m];
      });
      _block = from_list(_pipe$1);
      let messages_dict = _block;
      return [
        new Model(new Some(agent_id), messages_dict, model.draft_content, model.agents, model.connected),
        scroll_to_latest_message()
      ];
    } else if ($ instanceof ServerUpsertChatMessages) {
      let server_messages = $[1];
      let _block;
      let _pipe = model.messages;
      _block = merge(_pipe, from_list((() => {
        let _pipe$1 = server_messages;
        return map2(_pipe$1, (m) => {
          return [m.id, m];
        });
      })()));
      let messages = _block;
      ...
      let agents = $[0];
      return [
        new Model(model.current_agent, model.messages, model.draft_content, agents, model.connected),
        none()
      ];
    }
  } else {
    let $ = msg[0];
    if ($ instanceof TransportUp) {
      return [
        new Model(model.current_agent, model.messages, model.draft_content, model.agents, true),
        none()
      ];
    } else if ($ instanceof TransportDown) {
      return [
        new Model(model.current_agent, model.messages, model.draft_content, model.agents, false),
        none()
      ];
    } else {
      return [model, none()];
...
function init2(_) {
  return [
    new Model(new None, make(), "", hardcoded_agents, false),
    none()
  ];
}
function chat() {
  let encoder_decoder = new EncoderDecoder((msg) => {
    if (msg instanceof ClientMessage) {
      let message = msg[0];
      return new Ok2(encode_client_message(message));
    } else {
      return new Error2(undefined);
    }
  }, (encoded_msg) => {
  ...
  return component2(init2, update2, view, toList([]), encoder_decoder, websocket("http://localhost:8000/omni-app-ws"), (var0) => {
...
function main() {
  ...
  let _pipe$2 = try$(_pipe$1, (text4) => {
    ...
    if ($2) {
      ...
      return new Ok2(text4);
    }
  });
  let _pipe$3 = try$(_pipe$2, (_) => {
    return new Ok2(new Model(new None, make(), "", toList([]), false));
  });
  _block = from_result(_pipe$3);
  let init_model = _block;
  let $ = start4(chat(), "#app", init_model);
  if (!($ instanceof Ok2)) {
    throw makeError("let_assert", FILEPATH, "client", 33, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 677, end: 741, pattern_start: 688, pattern_end: 693 });
  }
  return;
}
